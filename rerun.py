...
class WaybackArchive:
    ...
    def get_link(self)->None:
        with open('last_link.txt','r') as f:
            self.last_link = f.read()

    # This method will make sure it starts from the right place
    def organize_list(self):
        location = self.found_links.index(self.last_link)
        self.found_links = self.found_links[location:]
